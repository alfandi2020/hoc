...
public function task()
{
	...
	if (strpos($a, '401')!== false) {
		//pagination settings
		$config['base_url'] = site_url('app/task');
		$config['total_rows'] = $this->m_app->memo_count($this->session->userdata('nip'));
		$config['per_page'] = "10";
		$config["uri_segment"] = 3;
		$choice = $config["total_rows"]/$config["per_page"];
		...
		$this->pagination->initialize($config);
		$data['page'] = ($this->uri->segment(3)) ? $this->uri->segment(3) : 0;
		$data['users_data'] = $this->m_app->task_get($config["per_page"], $data['page'], $this->session->userdata('nip'));
		$data['pagination'] = $this->pagination->create_links();
...
public function user()
{
	...
	if (strpos($a, '401')!== false) {
		...
		//inbox notif
		$nip = $this->session->userdata('nip');
		$sql = "SELECT COUNT(Id) FROM users where status='0'";
		$query = $this->db->query($sql);
		$res2 = $query->result_array();
		$result = $res2[0]['COUNT(Id)'];
		$data['count_inbox'] = $result;
